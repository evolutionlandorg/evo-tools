...
{ # Prevent execution if this script was only partially downloaded
  ...
  { have nixos-version; } && {
    ...
    echo >&2 "go to https://github.com/evolutionlandorg/furnace/ and follow manual installation instructions."
    exit 1
  }
  ...
  { have nix; } || {
    echo "evo-cli uses the Nix package manager to install programs behind the scenes."
  ...
  # nix-env -i -f https://github.com/evolutionlandorg/evo-tools/tarball/master 
  nix-env -i -f ~/workspace/contract/evolutionlandorg/evo-tools 
  # Finished!
  cat <<EOF

Installation finished!

You now have access to evo-cli.

Please logout and log back in to start using evo-cli, or run
